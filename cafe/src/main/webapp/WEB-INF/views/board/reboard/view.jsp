        ...
        $(document).ready(function() {
        	$('#newBtn').click(function() {
        		if('${userInfo == null}' == 'true') {
	        		...
	        		paraForm.attr('action', '${root}/reboard/write.cafe').attr('method', 'get');
	        		paraForm.submit();
        		}
        	});
        	...
        	$('#reBtn').click(function() {
        		if('${userInfo == null}' == 'true') {
	        		...
	        		var paraForm = $('#paraForm');
	        		paraForm.attr('action', '${root}/reboard/reply.cafe').attr('method', 'get');
	        		paraForm.submit();
        		}
        	});
        	...
        	$('#modiBtn').click(function() {
        		if('${userInfo == null}' == 'true') {
	        		...
	        		$(document).find('[name="seq"]').val('${article.seq}');
	        		var paraForm = $('#paraForm');
	        		paraForm.attr('action', '${root}/reboard/modify.cafe').attr('method', 'get');
	        		paraForm.submit();
        		}
        	});
        	...
        	$('#delBtn').click(function() {
        		if('${userInfo == null}' == 'true') {
	        		...
	        		if(confirm("글을 삭제하시겠습니까?")) {
		        		...
		        		$(document).find('[name="seq"]').val('${article.seq}');
		        		var paraForm = $('#paraForm');
		        		paraForm.attr('action', '${root}/reboard/delete.cafe').attr('method', 'get');
		        		paraForm.submit();
	        		}
        		}
        	});
        	...
        	$('#firstBtn').click(function() {
        		...
        		$(document).find('[name="word"]').val('');
        		var paraForm = $('#paraForm');
        		paraForm.attr('action', '${root}/reboard/list.cafe').attr('method', 'get');
        		paraForm.submit();
        	});
        	...
        	$(document).on('click', '#memolist>div .mdelBtn', function() {
        		if('${userInfo == null}' == 'true') {
	        		...
	        		$.ajax({
	       				...
	       				contentType:'application/json;charset=utf-8',
	       				dataType:'json',
	    				data : {'seq' : '${article.seq}', 'mseq' : mseq},
	       				success:function(response) {
	       					makeMemoList(response);
	       				},
	       				error:function(xhr,status,msg){
