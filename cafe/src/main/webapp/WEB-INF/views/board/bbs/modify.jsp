		...
		$(document).ready(function() {
			...
			$('#writeBtn').click(function() {
				...
				if($.trim($('#subject').val()) == '') {
	        		...
	        		$(document).find('[name="saveFolder"]').val('${article.saveFolder}');
	        		$(document).find('[name="saveFile"]').val('${article.saveFile}');
	        		$(document).find('[name="originalFile"]').val('${article.originalFile}');
	        		var writeForm = $('#modifyForm');
	        		writeForm.attr('action', '${root}/bbs/modify.cafe').attr('method', 'post');
	        		writeForm.submit();
				}
			});
			...
			$('#upf').on('change', function() {
	            if(this.files[0].size/1024/1024 > 50) { 
	            	alert("파일은 50mb까지만 가능합니다.");
			...
			$(".input-file").before(
				function() {
					if ( ! $(this).prev().hasClass('input-ghost') ) {
						var element = $("<input type='file' id='upf' class='input-ghost' style='visibility:hidden; height:0'>");
...
<!-- Center ======================================================================================= -->
        ...
        <div class="col-sm-9">
            ...
            <div class="form-group">
	        	<form id="modifyForm" class="form-horizontal" enctype="multipart/form-data" action="#">
	        		<input type="hidden" id="bcode" name="bcode"/>
		        	<input type="hidden" id="pageNo" name="pageNo"/>
		        	<input type="hidden" id="key" name="key"/>
		        	<input type="hidden" id="word" name="word"/>
		        	<input type="hidden" id="seq" name="seq"/>
		        	<input type="hidden" id="saveFolder" name="saveFolder"/>
                    ...
                    <fieldset>
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <input type="text" id="subject" name="subject" class="form-control" placeholder="제목" value="${article.subject}">
                            </div>
                        ...
                        <div class="col-sm-11">
                        	<textarea id="content" name="content" rows="10" cols="100" style="width:100%; height:412px;" placeholder="내용">${article.content}</textarea>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-2 control-label">파일</label>

                            <div class="col-sm-5 input-group input-file" name="upfile">
					    		<input type="text" class="form-control" placeholder='파일추가...' />			
            ...
            <div class="pull-right">
            	<button type="button" id="writeBtn" class="btn btn-sm btn-default" data-backdrop="static">글수정</button>
